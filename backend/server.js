...
// Get unique departments and positions - MUST be before /employees/:eid
empRouter.get('/employees/filters',
  async (req, res) => {
    try {
      ...
      res.status(500).json({ status: false, message: 'Failed to fetch filters' });
    }
});

// Search endpoint - MUST be before /employees/:eid to avoid route conflict
empRouter.get('/employees/search',
  async (req, res) => {
    const { department, position } = req.query;
    
    // Build query - allow empty strings to mean "all"
    let query = {};
